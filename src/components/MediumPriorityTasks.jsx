...
const MediumPriorityTasks = ({ backendUrl, userId }) => {
  ...
  const fetchMediumPriorityTasks = async () => {
    ...
    try {
      ...
      setError("An error occurred while fetching Medium-priority tasks.");
      console.error(err);
    } finally {
      setLoading(false);
    }
    ...
    try {
      const response = await fetch(`${backendUrl}/checklist/create`, {
        ...
        body: JSON.stringify({
          ...
          customItems: [
            { name: newTask.name, note: newTask.note, completed: false, priority: "medium" },
          ],
        }),
      });
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-semibold text-yellow-500">Medium Priority Tasks</h3>
        <button
      ...
      <div className="mt-4 color-white">
        {checklist.length > 0 ? (
          ...
          <p className="text-gray-500">No Medium-priority tasks available.</p>
        )}
      </div>
    </div>
  );
